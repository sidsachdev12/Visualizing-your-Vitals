class LineChart {
  ...
  updateVis() {
    ...
    */
    // draw line
    const lineSelectionworking = vis.svg.selectAll(".line-path-working")
      .data([vis.workingData]);

    lineSelectionworking.exit().remove();

    const lineEnterworking = lineSelectionworking.enter()
      .append("path")
      .attr("class", "line line-path");

    lineEnterworking.merge(lineSelectionworking)
      .transition()
      .duration(1000)
    ...
    /*
      RESTING -------------------------------------------------------------
    */
    const lineSelectionRest = vis.svg.selectAll(".line-path-rest")
      .data([vis.restData]);

    lineSelectionRest.exit().remove();

    const lineEnterRest = lineSelectionRest.enter()
      .append("path")
      .attr("class", "line line-path");

    lineEnterRest.merge(lineSelectionRest)
      .transition()
      .duration(1000)
      .attr("d", vis.lineGenerator)
      .attr("stroke", "brown")
    ...
    // add circles for each data point
    const circlesRest = vis.svg.selectAll(".dot")
    ...
    /*
      Meditating -------------------------------------------------------------
    */
    const lineSelectionMeditating = vis.svg.selectAll(".line-path-meditating")
      .data([vis.meditatingData]);

    lineSelectionMeditating.exit().remove();

    const lineEnterMeditating = lineSelectionMeditating.enter()
      .append("path")
      .attr("class", "line line-path");

    lineEnterMeditating.merge(lineSelectionMeditating)
      .transition()
      .duration(1000)
      .attr("d", vis.lineGenerator)
      .attr("stroke", "green")
    ...
    // add circles for each data point
    const circlesMeditating = vis.svg.selectAll(".dot")
      .data(vis.meditatingData, d => d.month);

    circlesMeditating.exit().remove();

    circlesMeditating.enter()
      .append("circle")
